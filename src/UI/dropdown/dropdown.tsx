...
const Dropdown = forwardRef(({options} : DropdownComponentProps, ref) => {
    ...
    const renderedOptions = options.map((option: DropdownProps, index) => {
        ...
        return <li className={`drop-down__element ${addBorderBottom ? "border-bottom" : ""}`} value={option.value} onClick={handleChange}>{option.label}</li>
    })
    ...
    return(
        ...
        {
            isOpen ? 
            <ul className="drop-down" style={{top: dropdownTop + "px", width: dropdownWidth + "px"}}>
                {renderedOptions}
            </ul>
            : null
        }
